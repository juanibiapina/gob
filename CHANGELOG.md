...
- TUI: restart, stop, kill, and delete keys (`r`/`s`/`S`/`d`) now work from any panel, not just the Jobs panel. In the Runs panel, `d` still deletes the selected run.
...
- Nix and flake support from the repository
