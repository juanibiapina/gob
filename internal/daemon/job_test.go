...
func TestJobManager_Cleanup(t *testing.T) {
	...
	job2, _ := jm.AddJob([]string{"cmd2"}, "/workdir2")
	time.Sleep(2 * time.Millisecond) // Ensure unique job IDs
	job3, _ := jm.AddJob([]string{"cmd3"}, "/workdir1")

	// Stop job1 and job3 (workdir1)
	executor.Handles()[0].Stop()
	executor.Handles()[2].Stop()
	time.Sleep(10 * time.Millisecond)

	// Cleanup workdir1 only
	count := jm.Cleanup("/workdir1")
	if count != 2 {
		t.Errorf("expected 2 cleaned, got %d", count)
	}

	// job2 should still exist
	_, err := jm.GetJob(job2.ID)
	if err != nil {
		t.Error("job2 should still exist")
	}

	// job1 and job3 should be gone
	_, err = jm.GetJob(job1.ID)
	if err == nil {
		t.Error("job1 should be removed")
	}
	_, err = jm.GetJob(job3.ID)
	if err == nil {
		t.Error("job3 should be removed")
	}
}

func TestJobManager_RunJob_NewJob(t *testing.T) {
	tmpDir := t.TempDir()
	executor := NewFakeProcessExecutor()
	...
	job, isRestart, err := jm.RunJob([]string{"echo", "hello"}, "/workdir")
	if err != nil {
	...
	if len(events) != 1 || events[0].Type != EventTypeJobAdded {
		t.Error("expected job_added event for new job")
	}
}

func TestJobManager_RunJob_ReusesStoppedJob(t *testing.T) {
	tmpDir := t.TempDir()
	executor := NewFakeProcessExecutor()
	...
	// Create initial job
	job1, _, _ := jm.RunJob([]string{"echo", "hello"}, "/workdir")
	...
	job2, isRestart, err := jm.RunJob([]string{"echo", "hello"}, "/workdir")
	if err != nil {
		t.Fatalf("RunJob failed: %v", err)
	}
	...
	if job2.ID != job1.ID {
		t.Error("expected same job ID on restart")
	}

	// Verify job_started event (not job_added)
	if len(events) != 1 || events[0].Type != EventTypeJobStarted {
		t.Errorf("expected job_started event for restart, got %v", events)
	}
}

func TestJobManager_RunJob_ErrorIfRunning(t *testing.T) {
	tmpDir := t.TempDir()
	executor := NewFakeProcessExecutor()
	jm := NewJobManagerWithExecutor(tmpDir, nil, executor)

	// Create running job
	jm.RunJob([]string{"echo", "hello"}, "/workdir")

	// Try to run same command while running
	_, _, err := jm.RunJob([]string{"echo", "hello"}, "/workdir")
	if err == nil {
